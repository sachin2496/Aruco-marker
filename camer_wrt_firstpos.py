...
#rvec and tvec of aruco marker wrt camera 
        ...
        if ret == 1:
            ...
            if len(corners)  > 0 :
                ...
                if f == 0 :
                    ...
                    tref = tvec
                revcmatrix = tf.euler_matrix(rvec[0][0][0] , rvec[0][0][1] , rvec[0][0][2] , 'sxyz')
                r_rel = np.dot(R_marker_ref,revcmatrix.T)
                t_rel = tref - tvec
                x = t_rel[0][0][0]
                y = t_rel[0][0][1]
                z = t_rel[0][0][2]
                quatern = tf.quaternion_from_matrix(r_rel)
                qx , qy , qz, qw = quatern
